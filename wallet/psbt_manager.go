...
func (w *Wallet) populatePsbtPacket(ctx context.Context, packet *psbt.Packet,
	...
	// dimensions of the new transaction.
...
func (w *Wallet) SignPsbt(ctx context.Context, params *SignPsbtParams) (
	*SignPsbtResult, error) {
	...
	for i := range packet.Inputs {
		...
		// unambiguous signing path. This is critical to ensure that we
		// have an unambiguous signing path. Our policy enforces that
		// an input should not contain conflicting Taproot and BIP32
		// derivation paths, nor multiple paths of the same type. This
		// prevents misinterpretations and ensures deterministic
		// signing.
		...
		// signing operation.
		signedInputs = append(signedInputs, uint32(i)) //nolint:gosec
	}
...
// fields is set, this function performs additional checks and returns nil if
// the UTXO information is missing or the index is out of bounds, preventing
// panics on malformed packets.
func fetchPsbtUtxo(packet *psbt.Packet, idx int) *wire.TxOut {
	if idx >= len(packet.Inputs) {
		log.Warnf("psbt input index %d out of bounds (len %d)",
			idx, len(packet.Inputs))
	...
	if pInput.WitnessUtxo != nil {
		return pInput.WitnessUtxo
	}

	if pInput.NonWitnessUtxo == nil {
		log.Warnf("input %d missing both WitnessUtxo and "+
			"NonWitnessUtxo", idx)
	...
	if idx >= len(packet.UnsignedTx.TxIn) {
		log.Warnf("psbt input index %d out of bounds for "+
			"UnsignedTx inputs (len %d)",
	...
	if int(prevIdx) < 0 ||
		int(prevIdx) >= len(pInput.NonWitnessUtxo.TxOut) {

		log.Warnf("input %d prevOut index %d out of bounds for "+
			"NonWitnessUtxo (len %d)",
			idx, prevIdx, len(pInput.NonWitnessUtxo.TxOut))

		return nil
	}

	return pInput.NonWitnessUtxo.TxOut[prevIdx]
}
...
func createTaprootSpendDetails(pInput *psbt.PInput,
	...
	case len(tapDerivation.LeafHashes) == 0:
	...
	case len(tapDerivation.LeafHashes) == 1:
	...
	default:
		return details, fmt.Errorf("%w: %d",
			ErrUnsupportedTaprootLeafCount,
			len(tapDerivation.LeafHashes))
	}
