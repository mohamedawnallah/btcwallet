...
//
// nolint:unparam - Different benchmarks intentionally use different parameter
// values.
...
func createTestWalletTxs(tb testing.TB, w *Wallet,
	...
	setSyncedToHeight(
		tb, w, highestBlockMeta.Block.Height,
		highestBlockMeta.Block.Hash,
	)
...
func createTxBatch(tb testing.TB, w *Wallet, txmgrNs,
	...
	for i := 0; i < count && i < len(addresses); i++ {
		...
		for j := 0; j < numOutputs; j++ {
			...
			// duplicate transaction errors when the same test data
			// is created multiple times. This is necessary for
		...
		for j := 0; j < numInputs; j++ {
			...
			if prevOutpoints != nil &&
				outpointIdx < len(prevOutpoints) {
				...
				// External input (no debit). Needed for tx to
				// be syntactically valid.
				prevHash := chainhash.Hash{}
				prevHash[0] = byte(i)
				prevHash[1] = byte(j)
				txIn := wire.NewTxIn(
					wire.NewOutPoint(&prevHash, uint32(j)),
					nil, nil,
				)
				tx.AddTxIn(txIn)
			}
