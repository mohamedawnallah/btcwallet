...
// TestPatchInputWithScript tests that patchInputWithScript correctly updates
// the PSBT input with the provided witness and/or sigScript.
func TestPatchInputWithScript(t *testing.T) {
	t.Parallel()
...
func TestFinalizeInput(t *testing.T) {
	...
	t.Run("error on missing utxo", func(t *testing.T) {
		t.Parallel()
		...
		// Assert: Expect error.
		require.ErrorContains(t, err, "input 0 has no UTXO info")
	})
}
